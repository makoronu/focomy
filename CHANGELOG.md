...
### Added
- カスタマイズ画面への直接導線追加
  - 管理画面サイドバー「外観」セクションに「カスタマイズ」リンク追加
  - フロントエンド管理バーに「カスタマイズ」リンク追加
  - WordPressのようにサイトを見ながら直接カスタマイズ画面へアクセス可能に
...
### Fixed
- 0.1.111のインポートエラーを修正
  - render_template → render_theme（正しい関数名に修正）

## [0.1.111] - 2026-01-16

### Fixed
- フォームページで管理バーが表示されない問題を修正
  - forms.pyでrender_themeを使用するよう変更
